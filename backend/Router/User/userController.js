...
exports.signUp = async (req, res) => {
    const {email, passwd, userName, department, sex, studentId, nickname} = req.body;

    // 어느하나라도 제대로 입력되지 않았을 때
    if (!email || !passwd || !userName || !department || !sex || !studentId || !nickname)
...
exports.nicknameCheck = async (req, res) => {
    ...
    if(nicknameCheckResult){
        return res.send(
            basicResponse(baseResponseStatus.NICKNAME_EXISTS)
        );
    }
    ...
    return res.send(
        basicResponse(baseResponseStatus.NICKNAME_CONFIRM_SUCCESS)
    );
};